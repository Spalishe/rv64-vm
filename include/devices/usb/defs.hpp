...
union xhci_usbcmd
{
	struct
	{
		...
		uint32_t css : 1;	 // Controller Save S;tate
...
struct xhci_interrupter
{
	...
	uint64_t ring_end{ 0 };
	uint32_t ring_size_trbs{ 0 };
	uint64_t event_enqueue_ptr = 0;
	bool pcs				   = true;
};
...
struct xhci_slot
{
	std::shared_ptr<USBDevice> attached_device;
	uint64_t ep0_tr_dequeue{ 0 };
	bool ep0_pcs{ true };
	uint8_t state{ 0 };
	uint32_t slot_id;
